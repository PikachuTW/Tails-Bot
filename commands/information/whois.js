        ...
        message.reply({
            embeds: [
                new MessageEmbed()
                    ...
                    .setDescription(`<@${target.id}>\n**加入日期:** <t:${Math.round(target.joinedTimestamp / 1000)}>\n**創建日期:** <t:${Math.round(target.user.createdTimestamp / 1000)}>\n**自我介紹:** \`${!introdata ? '無' : introdata.intro}\``)
                    .addField('伺服器權限', `\`${perm.filter((d) => target.permissions.has(d[0])).map((d) => d[1]).join('` `') || '無'}\``)
