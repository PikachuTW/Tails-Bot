...
exports.run = async (client, message, args) => {
  let target = message.mentions.members.first() || message.guild.members.cache.find(member => member.id === args[0]);
  if (!target) return message.reply('請給予有效目標!');

  let mee6data = await Mee6LevelsApi.getUserXp('828450904990154802', target.id);
  ...
  let tcdata = creditdata.tails_credit == Infinity ? Infinity : millify(creditdata.tails_credit)

  let lttdata = unbdata.total == Infinity ? Infinity : millify(unbdata.total)
