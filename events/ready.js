...
module.exports = async client => {
  ...
  let versionDate = await misc.findOne({ 'key': 'version' });
  ...
  setInterval(() => {
    let count = targetGuild.memberCount;
  ...
  setInterval(async () => {
    ...
    let active_levelData = await level.find({ 'discordid': { $in: targetGuild.members.cache.filter(member => member.roles.cache.find(role => role.id == '856808847251734559')).map(m => m.id) } });
    ...
    let s_active_levelData = await level.find({ 'discordid': { $in: targetGuild.members.cache.filter(member => member.roles.cache.find(role => role.id == '861459068789850172')).map(m => m.id) } });
