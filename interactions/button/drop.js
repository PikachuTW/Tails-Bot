...
const ACTIVE_ROLE_ID = '856808847251734559';

module.exports = async (client, interaction) => {
    const timestamp = interaction.message.createdTimestamp;
    const userId = interaction.member.id;
    const dropRes = await dropModel.findOne({ timestamp });
    if (!dropRes) {
        return interaction.reply({ content: '這個獎勵早已過時', ephemeral: true });
    }
    if (dropRes.claimed.includes(userId)) {
        return interaction.reply({ content: '你早就領取過獎勵!!', ephemeral: true });
    }
    ...
    const giveAmount = Math.floor(Math.round(economyData.level ** 1.225) * multi) + 1;
    ...
    if (interaction.member.roles.cache.has(ACTIVE_ROLE_ID) && Math.random() < 0.00125 * Math.floor(economyData.level / 20)) {
        const id = uuidv4();
        ...
        await interaction.member.send(`恭喜你得到了: 投資${translate[type]}BOOST 6小時，你的兌換碼:\n||${id}||`);
        await interaction.channel.send(`恭喜 ${interaction.member} 得到了投資${translate[type]}BOOST 6小時!!`);
    }
